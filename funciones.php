<?php
	...
	function cargarPagina($pagina){
		...
		if (!in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
		   	$e_comm->printMessage('El plugin Woocommerce no está disponible, no podrá utilizar Envialia Carrier hasta que lo instale y/o active');
		}
		...
		if (!class_exists('WC_Envialia_Shipping_Method')){
			class WC_Envialia_Shipping_Method extends WC_Shipping_Method{
				...
				public function calculate_shipping($package){
					global $e_comm, $woocommerce;

					$servicios = array('E72' => 'Envialia 72h', 'E24' => 'Envialia 24h', 'EEU' => 'Envialia EU Express', 'EWW' => 'Envialia Worldwide');
					...
					$num_articulos_bultos = (int) (obtenerOpcion('articulos_bulto') > 0)? obtenerOpcion('articulos_bulto'):0;
					...
					$numero_bultos = (int) $e_comm->bultosPorEnvio($numero);
					...
					foreach ($servicios as $servId => $servName) {
						if (obtenerOpcion('servicio_'.$servId)){ // El servicio está activo
							...
							if (!$e_comm->esPeninsular($pais) && ($servId=='E24' || $servId=='E72')) continue;
							else if ($e_comm->esPeninsular($pais) && ($servId=='EEU' || $servId=='EWW')) continue;
							else if (!$e_comm->esEuropeo($pais) && $servId=='EEU') continue;
		        			...
		        			if (!$envio_gratis){ // El envio no es gratis
			        			if($coste_fijo>0){ // El envio tiene un precio fijo
			        				...
			        				if($tipo_tarifa) $coste_envio = $e_comm->dameTarifa('importe', $servId, $pais, $cod_postal, $total);
			        				else $coste_envio = $e_comm->dameTarifa('peso', $servId, $pais, $cod_postal, $peso);
			        			...
			        			$coste_envio *= $numero_bultos; // Multiplicamos el coste por el número de bultos
		        				if ($e_comm->agregarImpuesto($pais)) $coste_envio += ($coste_envio * $impuesto)/100; // Sumamos los impuestos si procede
			        		}
	...
	if (!class_exists('envialia')) {
		class envialia {
		    ...
		    function dameTarifa($tipo, $servicio, $pais, $cp, $valor){ // peso | importe
		        ...
		        // Calculamos el precio según peso
		        if($tipo=='peso' && $valor >= $valor_max){
		            $peso_restante = $peso-$valor_max;

		...
		<?php
			foreach($select as $result){
				...
				echo '<tr><td><a href="'.admin_url('post.php?post='.$result->id_envio_order.'&action=edit').'" title="Ver pedido">#'.$result->id_envio_order.'</a></td><td>'.$result->fechaEsp.'</td><td>'.$result->codigo_envio.'</td><td>'.$result->num_albaran.'</td><td>
						<a href="'.$linkLabel.'" title="Etiquetas" class="envAction"><img src="'.plugins_url('envialia-carrier-for-woocomerce/img/sticker.png').'" /></a>
						...
						<a href="'.admin_url('admin.php?page=envialia-carrier-panel&order='.$result->id_envio_order.'&albaran='.$result->num_albaran.'&action=delete').'" title="Cancelar / Eliminar" class="envAction prompt"><img src="'.plugins_url('envialia-carrier-for-woocomerce/img/cancel.png').'" /></a>
					  </td></tr>';
			}
